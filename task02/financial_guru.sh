...
VOL () {
[[ $1 < '2015' ]] && echo "Error: year must start from 2015" > /dev/stderr && exit 1
[[ $2 > '2021' ]] && echo "Error: year must end at 2021" > /dev/stderr && exit 1

if [[ $2 == '2021' ]] && [[ $3 != 'Jan' ]] && [[ $3 != 'Feb' ]]
then
   echo "Error: no data after 02.2021"
   exit 1
fi
for i in $(seq $1 $2)
do
   MIN=$(echo "$DATE_VAL_TUPLE" | grep "$i"-"$3" | cut -d, -f2 | cut -d] -f1 | jq -s min)
   MAX=$(echo "$DATE_VAL_TUPLE" | grep "$i"-"$3" | cut -d, -f2 | cut -d] -f1 | jq -s max)
   DIFF=$(awk "BEGIN {print $MAX - $MIN} ")
   echo $3 $i "volatility is" $DIFF > /dev/stderr
   echo $3 $i "is the least volatile:" $DIFF
done
echo "=====================================" > /dev/stderr
